...
//0.369798725 0.589
  ...
  sprintf(FileWithZList, "/galformod/scratch/bmh20/Workspace/CosmologyTables/zlist_%04d_%04d.txt", om_Nbin,
          s8_Nbin);
      ...
      if(Halo[i].SnapNum <= LastSnapShotNr)
        {
          if(Halo[i].M_Crit200 > 1.e-8)
            Halo[i].M_Crit200 =
              Halo[i].M_Crit200 * ScaleMass * c_correction(Halo[i].M_Crit200, Halo[i].SnapNum);
          if(Halo[i].M_Mean200 > 1.e-8)
            Halo[i].M_Mean200 =
              Halo[i].M_Mean200 * ScaleMass * c_correction(Halo[i].M_Mean200, Halo[i].SnapNum);
          Halo[i].Vmax =
            Halo[i].Vmax * sqrt(ScaleMass / ScalePos) * sqrt(AA_OriginalCosm[Halo[i].SnapNum] /
                                                             AA[Halo[i].SnapNum]);
          ...
          for(j = 0; j < 3; j++)
            {
              ...
              Halo[i].Spin[j] *=
                ScalePos * sqrt(ScaleMass / ScalePos) * sqrt(AA[Halo[i].SnapNum] /
                                                             AA_OriginalCosm[Halo[i].SnapNum]);
              ...
              if(i != Halo[i].FirstHaloInFOFgroup)      // subhalos
                {
                  ...
                  dv *=
                    sqrt(ScaleMass / ScalePos) * sqrt(AA_OriginalCosm[Halo[i].SnapNum] / AA[Halo[i].SnapNum]);
...
double dgrowth_factor_dt(double a, double omega_m, double omega_l)
{
  ...
  do_m =
    -3 * omega_m * omega_l / (a * a * a * a) / (omega_m / (a * a * a) + omega_l) / (omega_m / (a * a * a) +
                                                                                    omega_l);
  do_l = -do_m;
  ...
  extra_fac = -(4 / 7. * pow(o_m, -3. / 7) * do_m - do_l
                ...
                / (1 + o_l / 70.) / (1 + o_l / 70)) / (pow(o_m,
                                                           4.0 / 7.0) - o_l + (1.0 + 0.5 * o_m) * (1.0 +
                                                                                                   o_l /
                                                                                                   70.0)) /
    (pow(o_m, 4.0 / 7.0) - o_l + (1.0 + 0.5 * o_m) * (1.0 + o_l / 70.0));

  dg =
    2.5 * do_m / (pow(o_m, 4.0 / 7.0) - o_l + (1.0 + 0.5 * o_m) * (1.0 + o_l / 70.0)) + 2.5 * o_m * extra_fac;
