                           ...
                           double FluxInputSSP[SSP_NAGES][SSP_NLambda], int MetalLoop)
{
  ...
  sprintf(buf1, "%s/FullSEDs/%s_%s_FullSED_m%0.4f.dat", SpecPhotDir, SSP, SpecPhotIMF,
  ...
  for(ageloop = 0; ageloop < SSP_NAGES; ageloop++)
    {
      for(i = 0; i < SSP_NLambda; i++)
        {
          fscanf(fa, "%lf %lf %lf %lf\n", &age, &Dumb1, &LambdaInputSSP[ageloop][i],
                 &FluxInputSSP[ageloop][i]);
          FluxInputSSP[ageloop][i] =
            1e11 * FluxInputSSP[ageloop][i] * LambdaInputSSP[ageloop][i] * LambdaInputSSP[ageloop][i] / (C *
                                                                                                         1.e8);
        }
