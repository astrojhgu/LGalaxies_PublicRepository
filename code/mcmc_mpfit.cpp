...
static int mp_fdjac2(mp_func funct,
                     ...
                     int *qulimited, double *ulimit, int *ddebug, double *ddrtol, double *ddatol);
static void mp_qrfac(int m, int n, double *a, int lda,
                     int pivot, int *ipvt, int lipvt, double *rdiag, double *acnorm, double *wa);
...
int mpfit(mp_func funct, int m, int npar,
          double *xall, mp_par * pars, mp_config * config, void *private_data, mp_result * result)
{
  mp_config conf;
  ...
  if((npar <= 0) || (conf.ftol <= 0) || (conf.xtol <= 0) ||
     (conf.gtol <= 0) || (conf.maxiter < 0) || (conf.stepfactor <= 0))
    {
      info = MP_ERR_PARAM;
      goto CLEANUP;
    }
...
OUTER_LOOP:
  ...
  iflag = mp_fdjac2(funct, m, nfree, ifree, npar, xnew, fvec, fjac, ldfjac,
                    ...
                    step, dstep, mpside, qulim, ulim, ddebug, ddrtol, ddatol);
  if(iflag < 0)
    {
      goto CLEANUP;
    }
