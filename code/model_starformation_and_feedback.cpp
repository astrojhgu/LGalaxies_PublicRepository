  ...
  if(FeedbackReheatingModel == 0)
    {
      ...
      if(reheated_mass * Gal[Gal[p].CentralGal].Vvir * Gal[Gal[p].CentralGal].Vvir >
         *stars * (EtaSNcode * EnergySNcode))
        reheated_mass =
          *stars * (EtaSNcode * EnergySNcode) / (Gal[Gal[p].CentralGal].Vvir * Gal[Gal[p].CentralGal].Vvir);
    }
...
void update_from_star_formation(int p, double stars, bool flag_burst, int nstep)
{
  ...
  if(Gal[p].DiskMass + stars_to_add > 1.e-8)
    for(i = 0; i < 3; i++)
      Gal[p].StellarSpin[i] =
        ((Gal[p].StellarSpin[i]) * (Gal[p].DiskMass) + stars_to_add * Gal[p].GasSpin[i]) / (Gal[p].DiskMass +
                                                                                            stars_to_add);
  ...
  Gal[p].sfh_MetalsDiskMass[Gal[p].sfh_ibin] =
    metals_add(Gal[p].sfh_MetalsDiskMass[Gal[p].sfh_ibin], Gal[p].MetalsColdGas, fraction);
#ifdef INDIVIDUAL_ELEMENTS
  Gal[p].sfh_ElementsDiskMass[Gal[p].sfh_ibin] =
    elements_add(Gal[p].sfh_ElementsDiskMass[Gal[p].sfh_ibin], Gal[p].ColdGas_elements, fraction);
#endif
#ifdef TRACK_BURST
  if(flag_burst)
...
void SN_feedback(int p, int centralgal, double stars, const char feedback_location[])
{
  ...
  if(strcmp(feedback_location, "HotGas") == 0)
    reheated_mass = 0.;
  else if(strcmp(feedback_location, "ColdGas") == 0)
    {
      CentralVvir = Gal[centralgal].Vvir;       // main halo Vvir
      MergeCentralVvir = Gal[Gal[p].CentralGal].Vvir;   //central subhalo Vvir
      ...
      if(FeedbackReheatingModel == 0)
        {
          ...
          if(reheated_mass * Gal[Gal[p].CentralGal].Vvir * Gal[Gal[p].CentralGal].Vvir >
             stars * (EtaSNcode * EnergySNcode))
            reheated_mass =
              stars * (EtaSNcode * EnergySNcode) / (Gal[Gal[p].CentralGal].Vvir *
                                                    Gal[Gal[p].CentralGal].Vvir);
        }
      /*else if(FeedbackReheatingModel == 1)
         {
         reheated_mass =  ALTERNATIVE Reheating LAW ;
  ...
  if(FeedbackEjectionModel == 0)
    {
      ejected_mass = (FeedbackEjectionEfficiency * (EtaSNcode * EnergySNcode) * stars *
                      min(1. / FeedbackEjectionEfficiency,
                          .5 + 1 / pow(EjectVmax / EjectPreVelocity,
                                       EjectSlope)) -
  ...
  else if(FeedbackEjectionModel == 1)   //the ejected material is assumed to have V_SN
    {
      ...
      ejected_mass =
        (SN_Energy - Reheat_Energy) / (0.5 * FeedbackEjectionEfficiency * (EtaSNcode * EnergySNcode));
  ...
  if(Gal[p].ColdGas > 0.)
    {
      ...
      if(Gal[p].Type == 0)
        {
          transfer_gas(p, "Hot", p, "Cold", ((float) reheated_mass) / Gal[p].ColdGas, "update_from_feedback",
                       __LINE__);
        }
      else if(Gal[p].Type < 3)
        {
          ...
          dis =
            separation_gal(centralgal, Gal[p].CentralGal) / (1 + ZZ[Halo[Gal[centralgal].HaloNr].SnapNum]);
  ...
  if(Gal[Gal[p].CentralGal].HotGas > 0.)
    {
      ...
      if(Gal[Gal[p].CentralGal].Type == 1)
          ...
          if(FateOfSatellitesGas == 0)
          ...
          else if(FateOfSatellitesGas == 1)
            {
              if(dis < Gal[centralgal].Rvir)
                transfer_gas(centralgal, "Hot", Gal[p].CentralGal, "Hot", fraction, "update_from_feedback",
                             __LINE__);
              else
                transfer_gas(Gal[p].CentralGal, "Ejected", Gal[p].CentralGal, "Hot", fraction,
                             "update_from_feedback", __LINE__);
        ...
        transfer_gas(centralgal, "Ejected", Gal[p].CentralGal, "Hot", fraction, "update_from_feedback",
                     __LINE__);
     ...
     || (Gal[p].BulgeMass + stars == 0 && Gal[p].BulgeSize > 1.e-9))
    {
      ...
      printf
        ("GasDiskMass=%e GasDiskSize=%e \nStellarDiskMass=%e StellarDiskSize=%e \nBulgeMass=%e Bulgesize=%e\n",
         Gal[p].ColdGas, Gal[p].GasDiskRadius, Gal[p].DiskMass, Gal[p].StellarDiskRadius, Gal[p].BulgeMass,
         Gal[p].BulgeSize);
