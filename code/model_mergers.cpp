...
double estimate_merging_time(int halonr, int mother_halonr, int p)
{
  ...
  if(SatelliteMass > 0.0)
    {
      mergtime = 1.17 * SatelliteRadius * SatelliteRadius * get_virial_velocity(mother_halonr) / (coulomb * G * SatelliteMass); // Binney & Tremaine Eq.7.26
  ...
  if((Gal[merger_centralgal].BulgeMass > 1.e-6 && Gal[merger_centralgal].BulgeSize == 0.0) ||
     (Gal[merger_centralgal].BulgeMass == 0.0 && Gal[merger_centralgal].BulgeSize > 1.e-6))
    {
      ...
      sprintf(sbuf,
              ...
              Gal[merger_centralgal].BulgeMass, Gal[merger_centralgal].BulgeSize,
              Gal[merger_centralgal].ColdGas, Gal[merger_centralgal].GasDiskRadius,
  ...
  if(Gal[merger_centralgal].ColdGas > 0.0)
    {
      BHaccrete = BlackHoleGrowthRate * mass_ratio
        / (1.0 +
           pow2((BlackHoleCutoffVelocity / Gal[merger_centralgal].Vvir))) * Gal[merger_centralgal].ColdGas;
      ...
      Gal[merger_centralgal].ColdGas_elements =
        elements_add(Gal[merger_centralgal].ColdGas_elements, Gal[merger_centralgal].ColdGas_elements,
                     -fraction);
#endif
    }
}
                           ...
                           double Mpstar, double Mpbulge, double Mpgas, double frac)
{
  ...
  if(mass_ratio < ThreshMajorMerger)
    {
      ...
      if(Mp > 0.0)
        Rp =
          (Gal[p].StellarDiskRadius / 3. * 1.68 * (Mpstar - Mpbulge) + Gal[p].BulgeSize * Mpbulge) / Mpstar;
      else
        Rp = 0.0;
    }
  /* Major Merger */
  else
    {
      ...
      if(Mc > 0.0)
        Rc =
          (Gal[merger_centralgal].StellarDiskRadius / 3. * 1.68 * (Mcstar - Mcbulge) +
           Gal[merger_centralgal].BulgeSize * Mcbulge +
           Gal[merger_centralgal].GasDiskRadius * frac * Mcgas / 3. * 1.68) / (Mcgas * frac + Mcstar);
      else
        Rc = 0.0;
      /* and satellite Mp */
      ...
      if(Mp > 0.0)
        Rp =
          (Gal[p].StellarDiskRadius / 3. * 1.68 * (Mpstar - Mpbulge) + Gal[p].BulgeSize * Mpbulge +
           Gal[p].GasDiskRadius * frac * Mpgas / 3. * 1.68) / (Mpgas * frac + Mpstar);
      else
        Rp = 0.0;
    }
  ...
  if(Rc >= 1.e-8 && Rp >= 1.e-8)
    Gal[merger_centralgal].BulgeSize =
      (Mp + Mc) * (Mp + Mc) / (Mp * Mp / Rp + Mc * Mc / Rc + fint / c * Mp * Mc / (Rp + Rc));

  if(Rc >= 1.e-8 && Rp <= 1.e-8)
    Gal[merger_centralgal].BulgeSize =
      (Mp + Mc) * (Mp + Mc) / (Mc * Mc / Rc + fint / c * Mp * Mc / (Rp + Rc));
  ...
  if(Rc <= 1.e-8 && Rp >= 1.e-8)
    Gal[merger_centralgal].BulgeSize =
      (Mp + Mc) * (Mp + Mc) / (Mp * Mp / Rp + fint / c * Mp * Mc / (Rp + Rc));


  if((Mp + Mc > 0.0 && Gal[merger_centralgal].BulgeSize == 0.0)
     || (Mp + Mc == 0.0 && Gal[merger_centralgal].BulgeSize > 0.0))
    {
      char sbuf[1000];
      ...
      printf
        ("    sat: BulgeMass=%0.7f, BulgeSize=%0.7f, GasMass=%0.7f, GasSize=%0.7f, DiskMass=%0.7f StellarSize=%0.7f \n",
         Gal[p].BulgeMass, Gal[p].BulgeSize, Gal[p].ColdGas, Gal[p].GasDiskRadius, Gal[p].DiskMass,
         Gal[p].StellarDiskRadius);
      printf
        ("central: BulgeMass=%0.7f, BulgeSize=%0.7f, GasMass=%0.7f, GasSize=%0.7f, DiskMass=%0.7f StellarSize=%0.7f \n",
         Gal[merger_centralgal].BulgeMass, Gal[merger_centralgal].BulgeSize, Gal[merger_centralgal].ColdGas,
         Gal[merger_centralgal].GasDiskRadius, Gal[merger_centralgal].DiskMass,
         Gal[merger_centralgal].StellarDiskRadius);
