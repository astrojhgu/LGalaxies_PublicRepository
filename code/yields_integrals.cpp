...
void integrate_yields()
{
  ...
  double SNIIEjectedMasses_lower_actual, SNIIEjectedMasses_upper_actual, SNIITotalMetals_lower_actual,
    SNIITotalMetals_upper_actual;
#ifdef INDIVIDUAL_ELEMENTS
  double SNIIYields_lower_actual[NUM_ELEMENTS], SNIIYields_upper_actual[NUM_ELEMENTS];
#endif
  double AGBEjectedMasses_lower_actual, AGBEjectedMasses_upper_actual, AGBTotalMetals_lower_actual,
    AGBTotalMetals_upper_actual;
#ifdef INDIVIDUAL_ELEMENTS
          ...
          for(i = 0; i <= SFH_ibin[snap][step]; i++)    //LOOP OVER SFH BINS
            {
              ...
              if(width_in_timesteps < 1)
                width_in_timesteps = 1;
                  ...
                  for(Zi = 0; Zi < LIFETIME_Z_NUM; Zi++)        //LOOP OVER POSSIBLE INITIAL METALLICITIES
                    {
                      ...
                      if(Mi_upper_actual <= 0.0 || Mi_upper_actual > 120.0
                         || Mi_upper == LIFETIME_MASS_NUM - 1)
                        Mi_upper_actual = SNII_MAX_MASS;        //Mi_upper_actual can be       < 0.0 for the highest Zi, as lifetimes[5][149+1] = 0.0 (i.e. doesn't exist!)

                      if(Mi_lower_actual <= 0.85)
                        Mi_lower_actual = AGB_MIN_MASS; //No stars below 0.85 Msun contribute to chemical enrichment.
                      ...
                      if(lifetimeMasses[Mi_upper] >= SNII_MAX_MASS)
                        Mi_upper_actual = SNII_MAX_MASS;        //No stars of mass above max. SN-II progenitor assumed to exist.
...
#ifdef INDIVIDUAL_ELEMENTS
                              find_actual_ejecta_limits(2, Mi_lower_actual, Mi_upper_actual, Mi_lower_SNII,
                                                        Mi_upper_SNII, Zi_SNII,
                                                        &SNIIEjectedMasses_lower_actual,
                                                        &SNIIEjectedMasses_upper_actual,
                                                        &SNIITotalMetals_lower_actual,
                                                        &SNIITotalMetals_upper_actual,
                                  ...
                                  if(SNIIMasses[j] <= SNIA_MAX_MASS)    //For mass range where both SN-II and SN-Ia progenitors are possible
                                    {
                                      if(j != Mi_lower_SNII && j != Mi_upper_SNII)      //If mass bin j is NEITHER the lowest NOR highest mass bin to be integrated over.
                                      ...
                                      else if(j == Mi_lower_SNII && j == Mi_upper_SNII && Mi_lower_actual >= SNII_MIN_MASS)     //If mass bin j is BOTH the lowest AND highest mass bin to be integrated over.
                                        {
                                          ...
                                          NormSNIIMassEjecRate[(STEPS * snap) + step][i][Zi] +=
                                            (1.0 -
                                             A_FACTOR) * ((Mi_upper_actual -
                                                           Mi_lower_actual) *
                                                          ((SNIIEjectedMasses_lower_actual +
                                                            SNIIEjectedMasses_upper_actual) / 2.0));
                                          NormSNIIMetalEjecRate[(STEPS * snap) + step][i][Zi] +=
                                            (1.0 -
                                             A_FACTOR) * ((Mi_upper_actual -
                                                           Mi_lower_actual) * ((SNIITotalMetals_lower_actual +
                                                                                SNIITotalMetals_upper_actual)
                                                                               / 2.0));
                                          float Sum_NormSNIIYieldRate;

                                          Sum_NormSNIIYieldRate = 0.0;
#ifdef INDIVIDUAL_ELEMENTS
                                          int k;
                                          ...
                                          for(k = 0; k < NUM_ELEMENTS; k++)
                                            {
                                              ...
                                              if(k != 0 && k != 1)
                                                {
                                                  Sum_NormSNIIYieldRate +=
                                                    NormSNIIYieldRate[(STEPS * snap) + step][i][Zi][k];
                                                }
#else
                                              switch (k)
                                                {
                                      ...
                                      else if(j == Mi_lower_SNII && j == Mi_upper_SNII && Mi_lower_actual < SNII_MIN_MASS)      //If mass bin j is BOTH the lowest AND highest mass bin to be integrated over, AND 'Mi_lower_actual' is below min. mass for SNe-II. (Still only counts stars from SNII_MIN_MASS to Mi_upper_actual).
                                        {
                                          ...
                                          NormSNIIMassEjecRate[(STEPS * snap) + step][i][Zi] +=
                                            (1.0 -
                                             A_FACTOR) * ((Mi_upper_actual -
                                                           SNII_MIN_MASS) * ((SNIIEjectedMasses_lower_actual +
                                                                              SNIIEjectedMasses_upper_actual)
                                                                             / 2.0));
                                          NormSNIIMetalEjecRate[(STEPS * snap) + step][i][Zi] +=
                                            (1.0 -
                                             A_FACTOR) * ((Mi_upper_actual -
                                                           SNII_MIN_MASS) * ((SNIITotalMetals_lower_actual +
                                                                              SNIITotalMetals_upper_actual) /
                                                                             2.0));
                                          float Sum_NormSNIIYieldRate;
                                          ...
                                          for(k = 0; k < NUM_ELEMENTS; k++)
                                            {
                                              ...
                                              kk = k;
                                              NormSNIIYieldRate[(STEPS * snap) + step][i][Zi][k] +=
                                                (1.0 - A_FACTOR) * (Mi_upper_actual -
                                                                    SNII_MIN_MASS) *
                                                ((SNIIYields_lower_actual[k] +
                                                  SNIIYields_upper_actual[k]) / 2.0);
                                              if(k != 0 && k != 1)
                                                {
                                                  Sum_NormSNIIYieldRate +=
                                                    NormSNIIYieldRate[(STEPS * snap) + step][i][Zi][k];
                                      ...
                                      else if(j == Mi_lower_SNII && j != Mi_upper_SNII) //If mass bin j IS the lowest but IS NOT the highest mass bin to be integrated over.
                                        {
                                          ...
                                          NormSNIIMassEjecRate[(STEPS * snap) + step][i][Zi] +=
                                            (1.0 -
                                             A_FACTOR) * ((SNIIMasses[j + 1] -
                                                           Mi_lower_actual) *
                                                          ((SNIIEjectedMasses_lower_actual +
                                                            SNIIEjectedMasses[Zi_SNII][j + 1]) / 2.0));
                                          NormSNIIMetalEjecRate[(STEPS * snap) + step][i][Zi] +=
                                            (1.0 - A_FACTOR) * (SNIIMasses[j + 1] -
                                                                Mi_lower_actual) *
                                            ((SNIITotalMetals_lower_actual +
                                              SNIITotalMetals[Zi_SNII][j + 1]) / 2.0);
#ifdef INDIVIDUAL_ELEMENTS
                                          int k;
                                      ...
                                      else if(j == Mi_upper_SNII && j != Mi_lower_SNII) //If mass bin j IS NOT the lowest but IS the highest mass bin to be integrated over.
                                        {
                                          ...
                                          NormSNIIMassEjecRate[(STEPS * snap) + step][i][Zi] +=
                                            (1.0 -
                                             A_FACTOR) * ((Mi_upper_actual -
                                                           SNIIMasses[j]) * ((SNIIEjectedMasses[Zi_SNII][j] +
                                                                              SNIIEjectedMasses_upper_actual)
                                                                             / 2.0));
                                          NormSNIIMetalEjecRate[(STEPS * snap) + step][i][Zi] +=
                                            (1.0 -
                                             A_FACTOR) * ((Mi_upper_actual -
                                                           SNIIMasses[j]) * ((SNIITotalMetals[Zi_SNII][j] +
                                                                              SNIITotalMetals_upper_actual) /
                                                                             2.0));
#ifdef INDIVIDUAL_ELEMENTS
                                          int k;
                                  ...
                                  else  //For mass range where only SN-II progenitors are possible
                                    {
                                      if(j != Mi_lower_SNII && j != Mi_upper_SNII)
                                          ...
                                          for(k = 0; k < NUM_ELEMENTS; k++)
                                            {
                                              ...
                                              NormSNIIYieldRate[(STEPS * snap) + step][i][Zi][k] +=
                                                (SNIIMasses[j + 1] -
                                                 SNIIMasses[j]) * ((SNIIYields[Zi_SNII][kk][j] +
                                                                    SNIIYields[Zi_SNII][kk][j + 1]) / 2.0);
#else
                                              switch (k)
                                                {
                                      ...
                                      else if(j == Mi_lower_SNII && j == Mi_upper_SNII)
                                        {
                                          ...
                                          NormSNIIMassEjecRate[(STEPS * snap) + step][i][Zi] +=
                                            ((Mi_upper_actual -
                                              Mi_lower_actual) * ((SNIIEjectedMasses_lower_actual +
                                                                   SNIIEjectedMasses_upper_actual) / 2.0));
                                          NormSNIIMetalEjecRate[(STEPS * snap) + step][i][Zi] +=
                                            ((Mi_upper_actual -
                                              Mi_lower_actual) * ((SNIITotalMetals_lower_actual +
                                                                   SNIITotalMetals_upper_actual) / 2.0));
#ifdef INDIVIDUAL_ELEMENTS
                                          int k;
                                          ...
                                          for(k = 0; k < NUM_ELEMENTS; k++)
                                            {
                                              ...
                                              NormSNIIYieldRate[(STEPS * snap) + step][i][Zi][k] +=
                                                (Mi_upper_actual -
                                                 Mi_lower_actual) * ((SNIIYields_lower_actual[k] +
                                                                      SNIIYields_upper_actual[k]) / 2.0);
#else
                                              switch (k)
                                                {
                                  ...
                                  if(j != t_lower_lifetime && j != t_upper_lifetime)    //If lifetime bin j is NEITHER the lowest NOR the highest bin to be integrated over.
                                    {
                                      ...
                                      NormSNIaMetalEjecRate[(STEPS * snap) + step][i][Zi] =
                                        NormSNIaMassEjecRate[(STEPS * snap) + step][i][Zi];
#ifdef INDIVIDUAL_ELEMENTS
                                      int k;
...
#ifdef INDIVIDUAL_ELEMENTS
                              find_actual_ejecta_limits(4, Mi_lower_actual, Mi_upper_actual, Mi_lower_AGB,
                                                        Mi_upper_AGB, Zi_AGB, &AGBEjectedMasses_lower_actual,
                                                        &AGBEjectedMasses_upper_actual,
                                                        &AGBTotalMetals_lower_actual,
                                                        &AGBTotalMetals_upper_actual, AGBYields_lower_actual,
                                                        AGBYields_upper_actual);
#else
                              find_actual_ejecta_limits(4, Mi_lower_actual, Mi_upper_actual, Mi_lower_AGB,
                                                        Mi_upper_AGB, Zi_AGB, &AGBEjectedMasses_lower_actual,
                                                        &AGBEjectedMasses_upper_actual,
                                  ...
                                  if(j != Mi_lower_AGB && j != Mi_upper_AGB)
                                    {
                                      ...
                                      NormAGBMassEjecRate[(STEPS * snap) + step][i][Zi] +=
                                        ((AGBMasses[j + 1] -
                                          AGBMasses[j]) * ((AGBEjectedMasses[Zi_AGB][j] +
                                                            AGBEjectedMasses[Zi_AGB][j + 1]) / 2.0));
                                      NormAGBMetalEjecRate[(STEPS * snap) + step][i][Zi] +=
                                        (AGBMasses[j + 1] -
                                         AGBMasses[j]) * ((AGBTotalMetals[Zi_AGB][j] +
                                                           AGBTotalMetals[Zi_AGB][j + 1]) / 2.0);
#ifdef INDIVIDUAL_ELEMENTS
                                      int k;
...
#ifndef MAINELEMENTS
                                          kk = k;
                                          NormAGBYieldRate[(STEPS * snap) + step][i][Zi][k] +=
                                            (AGBMasses[j + 1] -
                                             AGBMasses[j]) * ((AGBYields[Zi_AGB][kk][j] +
                                                               AGBYields[Zi_AGB][kk][j + 1]) / 2.0);
#else
                                          switch (k)
                                            {
                                            case 0:
                                          ...
                                          && Mi_lower_actual <= AGB_MAX_MASS)
                                    {
                                      ...
                                      NormAGBMassEjecRate[(STEPS * snap) + step][i][Zi] +=
                                        ((Mi_upper_actual -
                                          Mi_lower_actual) * ((AGBEjectedMasses_lower_actual +
                                                               AGBEjectedMasses_upper_actual) / 2.0));
                                      NormAGBMetalEjecRate[(STEPS * snap) + step][i][Zi] +=
                                        (Mi_upper_actual -
                                         Mi_lower_actual) * ((AGBTotalMetals_lower_actual +
                                                              AGBTotalMetals_upper_actual) / 2.0);
                                      float Sum_NormAGBYieldRate;

                                      Sum_NormAGBYieldRate = 0.0;
#ifdef INDIVIDUAL_ELEMENTS
                                      int k;
                                      ...
                                      for(k = 0; k < NUM_ELEMENTS; k++)
                                        {
                                          ...
                                          NormAGBYieldRate[(STEPS * snap) + step][i][Zi][k] +=
                                            (Mi_upper_actual -
                                             Mi_lower_actual) * ((AGBYields_lower_actual[k] +
                                                                  AGBYields_upper_actual[k]) / 2.0);
                                          if(k != 0 && k != 1)
                                            {
                                              Sum_NormAGBYieldRate +=
                                                NormAGBYieldRate[(STEPS * snap) + step][i][Zi][k];
                                            }
#else
                                          switch (k)
          ...
          if(lifetimeMetallicities[i] < Z_in)
            {
              i++;
              if(i == LIFETIME_Z_NUM)
                Zi_bin = i - 1; //If galaxy's Z is higher than max Z from table, then just take max Z from table
            }
          else
            Zi_bin = i;
        }
...
{
  if(lifetime == 0.0)
    return LIFETIME_MASS_NUM - 1;       //If the bin 'touches now', then return max mass (ie: star of shortest lifetime) ie: bin for 120Msun
  else if(lifetime > lifetimes[Zi_bin][0])
    return 0;                   //If true lifetime is longer than max lifetime in table (shouldn't be), then return element number 0
  else
    {
      int Mi_bin;
      ...
      while(Mi_bin == -1)
        {
          if(lifetimes[Zi_bin][i] > lifetime)
            {
              i++;
              if(i == LIFETIME_MASS_NUM)
                Mi_bin = i;     //If lifetime is shorter than min lifetime from table, then just return max mass (120 Msun)
            }
          else
            Mi_bin = i;
        }
...
int max_Mi_lower(int Mi_lower, int channel_type)
{
  switch (channel_type)
    ...
    case 2:                    //SNII mass limits
      if(lifetimeMasses[Mi_lower] > SNII_MIN_MASS)
        return Mi_lower;
      else
        {
          int i;
    ...
    case 3:                    //SNIa mass limits
      if(lifetimeMasses[Mi_lower] > 0.85)
        return Mi_lower;        //NB: Lifetimes of SNe-Ia binaries depend on M2, not Mb. (ie: 0.85<=M2<=8.0)
      else
        {
          int i;
    ...
    case 4:                    //AGB mass limits
      if(lifetimeMasses[Mi_lower] > AGB_MIN_MASS)
        return Mi_lower;
      else
        {
          int i;
    ...
    default:
      printf
        ("Wrong ejection mode chosen in max_Mi_lower: Use either 2 (SNe-II), 3 (SNe-Ia) or 4 (AGB winds)");
      exit(1);
    }
}
...
int min_Mi_upper(int Mi_upper, int channel_type)
{
  switch (channel_type)
...
#ifndef DTD
    case 3:                    //SNIa mass limits
      if(lifetimeMasses[Mi_upper] < 8.0)
        return Mi_upper;        //NB: Lifetimes of SNe-Ia binaries depends on M2, not Mb. (ie: 0.85<=M2<=8.0)
      else
        {
          int i;
...
#endif
    case 4:                    //AGB mass limits
      if(lifetimeMasses[Mi_upper] < AGB_MAX_MASS)
        return Mi_upper;
      else
        {
          int i;
    ...
    default:
      printf
        ("Wrong ejection mode chosen in min_Mi_upper: Use either 2 (SNe-II), 3 (SNe-Ia) or 4 (AGB winds)");
      exit(1);
    }
}
...
{
  if(masslimit == SNII_MAX_MASS)
    return SNII_MASS_NUM - 1;
  else
    {
      int Mi_bin;
          ...
          if(SNIIMasses[i] < masslimit)
            {
              i++;
              if(i == SNII_MASS_NUM)
                Mi_bin = i - 1; //If mass is greater than max mass for SNe-II (shouldn't be), then just return max mass (120.0 Msun)
            }
          else
            Mi_bin = i;
        }
...
{
  if(masslimit == AGB_MAX_MASS)
    return AGB_MASS_NUM - 1;
  else
    {
      int Mi_bin;
          ...
          if(AGBMasses[i] < masslimit)
            {
              i++;
              if(i == AGB_MASS_NUM)
                Mi_bin = i - 1; //If mass is greater than max mass for AGB winds (shouldn't be), then just return max mass (5.0 Msun)
            }
          else
            Mi_bin = i;
        }
  ...
  switch (channel_type)
    ...
    case 2:                    //SNII
      if(Mi_lower == 0)
        ...
        {
          *EjectedMasses_lower_actual =
            SNIIEjectedMasses[Zi][Mi_lower] +
            ((SNIIEjectedMasses[Zi][Mi_lower + 1] -
              SNIIEjectedMasses[Zi][Mi_lower]) * ((Mi_lower_actual -
                                                   SNIIMasses[Mi_lower]) / (SNIIMasses[Mi_lower + 1] -
                                                                            SNIIMasses[Mi_lower])));
          *TotalMetals_lower_actual =
            SNIITotalMetals[Zi][Mi_lower] +
            ((SNIITotalMetals[Zi][Mi_lower + 1] -
              SNIITotalMetals[Zi][Mi_lower]) * ((Mi_lower_actual -
                                                 SNIIMasses[Mi_lower]) / (SNIIMasses[Mi_lower + 1] -
                                                                          SNIIMasses[Mi_lower])));
#ifdef INDIVIDUAL_ELEMENTS
          int k;
...
#ifndef MAINELEMENTS
              kk = k;
              Yields_lower_actual[k] =
                SNIIYields[Zi][kk][Mi_lower] +
                ((SNIIYields[Zi][kk][Mi_lower + 1] -
                  SNIIYields[Zi][kk][Mi_lower]) * ((Mi_lower_actual -
                                                    SNIIMasses[Mi_lower]) / (SNIIMasses[Mi_lower + 1] -
                                                                             SNIIMasses[Mi_lower])));
#else
              switch (k)
      ...
      if(Mi_upper == SNII_MASS_NUM - 1)
        ...
        {
          *EjectedMasses_upper_actual =
            SNIIEjectedMasses[Zi][Mi_upper] +
            ((SNIIEjectedMasses[Zi][Mi_upper + 1] -
              SNIIEjectedMasses[Zi][Mi_upper]) * ((Mi_upper_actual -
                                                   SNIIMasses[Mi_upper]) / (SNIIMasses[Mi_upper + 1] -
                                                                            SNIIMasses[Mi_upper])));
          *TotalMetals_upper_actual =
            SNIITotalMetals[Zi][Mi_upper] +
            ((SNIITotalMetals[Zi][Mi_upper + 1] -
              SNIITotalMetals[Zi][Mi_upper]) * ((Mi_upper_actual -
                                                 SNIIMasses[Mi_upper]) / (SNIIMasses[Mi_upper + 1] -
                                                                          SNIIMasses[Mi_upper])));
#ifdef INDIVIDUAL_ELEMENTS
          int k;
...
#ifndef MAINELEMENTS
              kk = k;
              Yields_upper_actual[k] =
                SNIIYields[Zi][kk][Mi_upper] +
                ((SNIIYields[Zi][kk][Mi_upper + 1] -
                  SNIIYields[Zi][kk][Mi_upper]) * ((Mi_upper_actual -
                                                    SNIIMasses[Mi_upper]) / (SNIIMasses[Mi_upper + 1] -
                                                                             SNIIMasses[Mi_upper])));
#else
              switch (k)
    ...
    case 4:                    //AGB
      if(Mi_lower == 0)
        ...
        {
          *EjectedMasses_lower_actual =
            AGBEjectedMasses[Zi][Mi_lower] +
            ((AGBEjectedMasses[Zi][Mi_lower + 1] -
              AGBEjectedMasses[Zi][Mi_lower]) * ((Mi_lower_actual -
                                                  AGBMasses[Mi_lower]) / (AGBMasses[Mi_lower + 1] -
                                                                          AGBMasses[Mi_lower])));
          *TotalMetals_lower_actual =
            AGBTotalMetals[Zi][Mi_lower] +
            ((AGBTotalMetals[Zi][Mi_lower + 1] -
              AGBTotalMetals[Zi][Mi_lower]) * ((Mi_lower_actual -
                                                AGBMasses[Mi_lower]) / (AGBMasses[Mi_lower + 1] -
                                                                        AGBMasses[Mi_lower])));
#ifdef INDIVIDUAL_ELEMENTS
          int k;
          ...
          for(k = 0; k < NUM_ELEMENTS; k++)
            {
              ...
              kk = k;
              Yields_lower_actual[k] =
                AGBYields[Zi][kk][Mi_lower] +
                ((AGBYields[Zi][kk][Mi_lower + 1] -
                  AGBYields[Zi][kk][Mi_lower]) * ((Mi_lower_actual -
                                                   AGBMasses[Mi_lower]) / (AGBMasses[Mi_lower + 1] -
                                                                           AGBMasses[Mi_lower])));
#else
              switch (k)
      ...
      if(Mi_upper == AGB_MASS_NUM - 1)
        ...
        {
          *EjectedMasses_upper_actual =
            AGBEjectedMasses[Zi][Mi_upper] +
            ((AGBEjectedMasses[Zi][Mi_upper + 1] -
              AGBEjectedMasses[Zi][Mi_upper]) * ((Mi_upper_actual -
                                                  AGBMasses[Mi_upper]) / (AGBMasses[Mi_upper + 1] -
                                                                          AGBMasses[Mi_upper])));
          *TotalMetals_upper_actual =
            AGBTotalMetals[Zi][Mi_upper] +
            ((AGBTotalMetals[Zi][Mi_upper + 1] -
              AGBTotalMetals[Zi][Mi_upper]) * ((Mi_upper_actual -
                                                AGBMasses[Mi_upper]) / (AGBMasses[Mi_upper + 1] -
                                                                        AGBMasses[Mi_upper])));
#ifdef INDIVIDUAL_ELEMENTS
          int k;
          ...
          for(k = 0; k < NUM_ELEMENTS; k++)
            {
              ...
              kk = k;
              Yields_upper_actual[k] =
                AGBYields[Zi][kk][Mi_upper] +
                ((AGBYields[Zi][kk][Mi_upper + 1] -
                  AGBYields[Zi][kk][Mi_upper]) * ((Mi_upper_actual -
                                                   AGBMasses[Mi_upper]) / (AGBMasses[Mi_upper + 1] -
                                                                           AGBMasses[Mi_upper])));
#else
              switch (k)
