  ...
  for(ii = 1; ii < ChainLength + 1; ii++)
    {
      ...
      time(&final);
      printf("Task %d chain %d (%d) took %lds\n", ThisTask + FirstChainNumber, CurrentMCMCStep,
             GlobalMCMCStep, final - local_initial);
      printf("Global Time Elapsed %lds, %f hours\n", final - global_starting_time,
             (final - global_starting_time) / 3600.);
  ...
  for(i = 0; i < MCMCNpar; i++)
    {
      ...
      if(MCMC_PAR[i].Sampling_Switch == 1)
        if(MCMC_PAR[i].Value[0] < MCMC_PAR[i].PriorMin || MCMC_PAR[i].Value[0] > MCMC_PAR[i].PriorMax)
          {
            printf("value=%0.6f priormin=%0.4f priormax=%0.4f\n", MCMC_PAR[i].Value[0], MCMC_PAR[i].PriorMin,
                   MCMC_PAR[i].PriorMax);
            char sbuf[1000];
        ...
        for(snap = 0; snap < NOUT; snap++)
          {
            if(Time_Dependent_PhysPar == 0 && snap > 0)
              ...
              {
                do
                  MCMC_PAR[i].PropValue[snap] =
                    MCMC_PAR[i].Value[snap] * exp(MCMC_LogStep_Size * gassdev(&MCMCseed));
...
void read_sample_info(void)
{
  ...
  if(Switch_MR_MRII == 1)
    {
      sprintf(buf, "%s/%ssample_allz_nh_Switch_MR_MRII_%d.dat", MCMCSampleDir, MCMCSampleFilePrefix,
              MCMCSampleFile);
      if(!(fa = fopen(buf, "r")))
        {
          char sbuf[1000];
  ...
  for(snap = 0; snap < NOUT; snap++)
    {
      ...
      for(i = 0; i < NFofsInSample[snap]; i++)
        {
          fscanf(fa, "%lld %d %d %lg\n", &MCMC_FOF[i].FoFID[snap], &DumbTreeNrColector, &DumbFileNrColector,
              ...
              for(j = 0; j < Nbins[snap][i]; j++)
                {
                  ...
                  if(strcmp(MCMC_Obs[i].TestType, "chi_sq") == 0
                     || strcmp(MCMC_Obs[i].TestType, "maxlike") == 0)
                    fscanf(fa, "%lg %lg %lg %lg", &MCMC_Obs[i].Bin_low[snap][j],
                           &MCMC_Obs[i].Bin_high[snap][j], &MCMC_Obs[i].Obs[snap][j],
                           &MCMC_Obs[i].Error[snap][j]);
                  //Binomial TESTS
                  else if(strcmp(MCMC_Obs[i].TestType, "binomial") == 0)
                    fscanf(fa, "%f %lg %lg", &BinValueColector, &MCMC_Obs[i].ObsUp[snap][j],
                           &MCMC_Obs[i].ObsDown[snap][j]);
                }
              fclose(fa);
