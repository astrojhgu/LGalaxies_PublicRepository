...
void halomodel(double *r_arr, double *proj_arr, float masslimit_low, float masslimit_high, int snap)
{
  ...
  for(i = 0; i < NK + extralin; i++)
    {
      ...
      PowerTable[i] =
        log10((p1h + p2h) / (gsl_spline_eval(ellipSpline, kPowerTable[i], ellipAcc) +
                             1) / (gsl_spline_eval(alignSpline, kPowerTable[i], alignAcc) + 1));
    }                           //for
  ...
  for(i = 0; i < NK + extralin; ++i)
    {
      ...
      if(k < 4 * PI / Rhalo_cc)
        {
          pcorr_cc =
            min(4 * PI / 3. * pow(Rhalo_cc, 3) * (pconv_W(k, Rhalo_cc, 0) + TopHatWindow(k * Rhalo_cc)),
                TwoPowerSpec(k, 0));
          if(Rhalo_cs > 0)
            pcorr_cs =
              min((mugal_qawo(k, 4 * PI / 3. * Delta * rho_mean * pow(Rhalo_cs, 3)) -
                   TopHatWindow(k * Rhalo_cs / Delta_invth)) * 4 * PI / 3. * pow(Rhalo_cs, 3) * (pconv_W(k,
                                                                                                         Rhalo_cs,
                                                                                                         1) +
                                                                                                 TopHatWindow
                                                                                                 (k *
          ...
          if(Rhalo_ss > 0)
            pcorr_ss =
              min((pow(mugal_qawo(k, 4 * PI / 3. * Delta * rho_mean * pow(Rhalo_ss, 3)), 2) -
                   pow(TopHatWindow(k * Rhalo_ss / Delta_invth), 2)) * 4 * PI / 3. * pow(Rhalo_ss,
                                                                                         3) * (pconv_W(k,
                                                                                                       Rhalo_ss,
                                                                                                       2) +
                                                                                               TopHatWindow(k
  ...
  for(i = 0; i < 1000; ++i)
    {
      fprintf(fd, "%g %g\n", pow(10., 0.001 * i * 5 - 2),
              pow(10.,
                  gsl_spline_eval(NewpowSpline, 0.001 * i * 5 - 2,
                                  NewpowAcc)) - gsl_spline_eval(TwopcorrSpline, 0.001 * i * 5 - 2,
                                                                TwopcorrAcc));
    }                           //for
  fclose(fd);
#endif
  ...
  for(i = 0; i < NR * 10; ++i)
    {
      ...
      CorrTable[i] =
        corr_qawo(rCorrTable[i], 0.01, 0.99) + corr_qawo(rCorrTable[i], 1., 99.) + corr_qawo(rCorrTable[i],
                                                                                             100.,
                                                                                             900.) +
  ...
  for(i = 0; i < 1000; ++i)
    {
      fprintf(fd, "%g %g\n", pow(10., 0.001 * i * 5.785 - 3),
              gsl_spline_eval(CorrSpline, pow(10., 0.001 * i * 5.785 - 3), CorrAcc));
    }                           //for
  fclose(fd);
#endif
  for(i = 0; i < NR; ++i)
  ...
  if(censat == 0)
    {
      if(k > 0.01)
        ...
        return
          pow(k, pow_ns) / pow(1e-4, pow_ns) * PowerSpec(1e-4) / PowerSpec(0.01) * pow(10.,
                                                                                       gsl_spline_eval
                                                                                       (Twopow_ccSpline, -2.,
                                                                                        Twopow_ccAcc));
    }                           //if
...
double pconv_W(double k, double R, int censat)
{
  ...
  F.params = &params;
  gsl_integration_qag(&F, log(pow(10., -8.)), log(k + kmax), 0, 1.0e-3, WORKSIZE, GSL_INTEG_GAUSS41, w,
                      &result, &abserr);
  gsl_integration_workspace_free(w);
...
double calc_mean_rhalo_simple(int censat, int norm)
{
  ...
  F.params = &params;
  gsl_integration_qag(&F, log(pow(10., cutoff_low[2 + censat])), log(pow(10., cutoff_high[2 + censat])), 0,
                      1.0e-3, WORKSIZE, GSL_INTEG_GAUSS41, w, &result, &abserr);
  gsl_integration_workspace_free(w);
  return result;
...
double NewPowerSpec(double k)
{
  ...
  return pow(10., gsl_spline_eval(NewpowSpline, log10(k), NewpowAcc)) - gsl_spline_eval(TwopcorrSpline,
                                                                                        log10(k),
                                                                                        TwopcorrAcc);
}                               //NewPowerSpec
...
#else
  gsl_integration_qag(&F, sqrt(pimin * pimin + sigma * sigma), sqrt(pimax * pimax + sigma * sigma), 0, 1.0e-3,
                      WORKSIZE, GSL_INTEG_GAUSS51, w, &result, &abserr);
#endif
  gsl_integration_workspace_free(w);
  return result;
}                               //proj_corr
...
double b(double m, int i)
{
  ...
  cT = 2.4;
  b1 =
    1 - AT * pow(nu2, 0.5 * aT) / (pow(nu2, 0.5 * aT) + pow(delta_c2, 0.5 * aT)) + BT * pow(nu2,
                                                                                            0.5 * bT) +
    CT * pow(nu2, 0.5 * cT);
  switch (i)
    {
...
double mugal_qawo(double k, double m)
{
  ...
  double
    norm =
    pc / (rvir3 * 4 * PI *
          exp(gsl_sf_lngamma(pa / pc) + log(gsl_sf_gamma_inc_P(pa / pc, pow(xmax / pb, pc)))));
...
{
  double value = gsl_spline_eval(paSpline, min(max(m, parscutoff_low), parscutoff_high), paAcc);
  ...
  if(i == 0)
    {                           //counterterm due to Valageas & Nishimichi (2011)
      if(j == 0)
        ...
        return
          nbargal(m) * NgalF(m, 5) / pow(ngal_mean,
                                         2) * (pow(mugal_qawo(k, m), 2) - pow(TopHatWindow(k * Radius(m)),
                                                                              2)) * m;
#endif
    }                           //if
  else
...
{                               //k=wavenumber, i=which haloterm [0/1], j=central/satellite [0/1/2]
  ...
  F.params = &params;
  gsl_integration_qag(&F, log(pow(10., cutoff_low[3 - i + j])), log(pow(10., cutoff_high[3 - i + j])), 0,
                      1.0e-3, WORKSIZE, GSL_INTEG_GAUSS41, w, &result, &abserr);
  gsl_integration_workspace_free(w);
  return result;
}                               //Mcensat
...
double ngal_mean_calc(int j)
{
  ...
  F.params = &params;
  gsl_integration_qag(&F, log(pow(10., cutoff_low[j])), log(pow(10., cutoff_high[j])), 0, 1.0e-3, WORKSIZE,
      ...
      for(j = 0; j < UsedFofsInSample[snap]; ++j)
        {
          ...
          if(MCMC_FOF2[j].M_Mean200[snap] > minfofmass + mbin * (maxfofmass - minfofmass) / (double) massbins
             &&
             MCMC_FOF2[j].M_Mean200[snap] <
             minfofmass + (mbin + 1) * (maxfofmass - minfofmass) / (double) massbins)
            {
              i = MCMC_FOF2[j].IndexOfCentralGal[snap];
              if(i >= 0)
                {
                  if(MCMC_GAL[HashTable[i]].StellarMass[snap] > masslimit_low
                     && MCMC_GAL[HashTable[i]].StellarMass[snap] <= masslimit_high)
                    ncen = 1;
                  for(k = 1; k < MCMC_GAL[HashTable[i]].ngal[snap]; ++k)
                    {
                      if(MCMC_GAL[HashTable[i + k]].StellarMass[snap] > masslimit_low
                         && MCMC_GAL[HashTable[i + k]].StellarMass[snap] <= masslimit_high)
                        nsat++;
                    }           //for
                }               //if
              ...
              for(jj = 1; jj < MCMC_GAL[HashTable[i]].ngal[snap]; ++jj)
                {
                  if(MCMC_GAL[HashTable[i + jj]].StellarMass[snap] > masslimit_low
                     && MCMC_GAL[HashTable[i + jj]].StellarMass[snap] <= masslimit_high)
                    {
                      found++;
                      relx =
                        min(fabs(MCMC_GAL[HashTable[i + jj]].x[snap] - MCMC_GAL[HashTable[i]].x[snap]),
                            fabs(boxsize -
                                 fabs(MCMC_GAL[HashTable[i + jj]].x[snap] - MCMC_GAL[HashTable[i]].x[snap])));
                      rely =
                        min(fabs(MCMC_GAL[HashTable[i + jj]].y[snap] - MCMC_GAL[HashTable[i]].y[snap]),
                            fabs(boxsize -
                                 fabs(MCMC_GAL[HashTable[i + jj]].y[snap] - MCMC_GAL[HashTable[i]].y[snap])));
                      relz =
                        min(fabs(MCMC_GAL[HashTable[i + jj]].z[snap] - MCMC_GAL[HashTable[i]].z[snap]),
                            fabs(boxsize -
                                 fabs(MCMC_GAL[HashTable[i + jj]].z[snap] - MCMC_GAL[HashTable[i]].z[snap])));
                      ...
                      for(jj = 1; jj < MCMC_GAL[HashTable[i]].ngal[snap]; ++jj)
                        {
                          if(MCMC_GAL[HashTable[i + jj]].StellarMass[snap] > masslimit_low &&
                             MCMC_GAL[HashTable[i + jj]].StellarMass[snap] <= masslimit_high)
                            {
                              relx =
                                min(fabs
                                    (MCMC_GAL[HashTable[i + jj]].x[snap] - MCMC_GAL[HashTable[i]].x[snap]),
                                    fabs(boxsize -
                                         fabs(MCMC_GAL[HashTable[i + jj]].x[snap] -
                                              MCMC_GAL[HashTable[i]].x[snap])));
                              rely =
                                min(fabs
                                    (MCMC_GAL[HashTable[i + jj]].y[snap] - MCMC_GAL[HashTable[i]].y[snap]),
                                    fabs(boxsize -
                                         fabs(MCMC_GAL[HashTable[i + jj]].y[snap] -
                                              MCMC_GAL[HashTable[i]].y[snap])));
                              relz =
                                min(fabs
                                    (MCMC_GAL[HashTable[i + jj]].z[snap] - MCMC_GAL[HashTable[i]].z[snap]),
                                    fabs(boxsize -
                                         fabs(MCMC_GAL[HashTable[i + jj]].z[snap] -
                                              MCMC_GAL[HashTable[i]].z[snap])));
                              r = sqrt(relx * relx + rely * rely + relz * relz);
                              if(r / rvir > xmin && r / rvir <= xmax)
                                {
